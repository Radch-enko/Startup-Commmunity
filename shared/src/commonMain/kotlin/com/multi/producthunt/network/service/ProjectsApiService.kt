...
interface ProjectsApiService {
    ...
    @Headers(["Content-Type: application/json"])
    @POST("users/update")
    fun updateUser(@Body body: UpdateUserBody, @Header("Authorization") token: String): Flow<ApiResult<UserResponse>>

    @Multipart
    @POST("users/profileImage")
    fun updateProfileImage(@Part profileImage: List<PartData>, @Header("Authorization") token: String): Flow<ApiResult<UserResponse>>

    @Multipart
    @POST("users/coverImage")
    fun updateCoverImage(@Part coverImage: List<PartData>, @Header("Authorization") token: String): Flow<ApiResult<UserResponse>>
}